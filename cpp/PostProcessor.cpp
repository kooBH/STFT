...
PostProcessor::PostProcessor(uint32_t _frame_size,
 ...
 *
 * */
short *PostProcessor::Overlap(double **in) {
    int i, j;
...
short *PostProcessor::Array2WavForm(double **in) {
    int i, j;
    for (i = 0; i < static_cast<int>(shift_size); i++) {
        for (j = 0; j < static_cast<int>(channels); j++) {
...
short *PostProcessor::Frame2Wav(double *in) {
    int i, j;
    for (i = 0; i < static_cast<int>(frame_size); i++) {
        for (j = 0; j < static_cast<int>(channels); j++) {
